...
interface HarnessParams {
  ...
  /** A0 对照实验：流式消息渲染形态（animated=现状 / static=关动画 / plain=转义纯文本绕过 XMarkdown） */
  streamRender: "animated" | "static" | "plain";
}
...
function parseParams(): HarnessParams {
  ...
  return {
    ...
    streamRender: streamRender === "static" || streamRender === "plain" ? streamRender : "animated",
  };
}
...
const perfWindow = window as typeof window & {
  ...
  /** A0 对照组开关：流式消息渲染形态（animated/static/plain），挂载前写入 */
